...
class dispute_user_avatars {
	...
	public function admin_init() {
		register_setting( 'discussion', 'dispute_user_avatars_caps', array( $this, 'sanitize_options' ) );
		add_settings_field( 'basic-user-avatars-caps', __( 'Profile Picture Permissions', 'basic-user-avatars' ), array( $this, 'avatar_settings_field' ), 'discussion', 'avatars' );
	}
	/**
	...
	public function avatar_settings_field( $args ) {
		$options = get_option( 'dispute_user_avatars_caps' );
		?>
		<label for="dispute_user_avatars_caps">
			<input type="checkbox" name="dispute_user_avatars_caps" id="dispute_user_avatars_caps" value="1" <?php checked( $options['dispute_user_avatars_caps'], 1 ); ?>/>
	...
	public function sanitize_options( $input ) {
		$new_input['dispute_user_avatars_caps'] = empty( $input['dispute_user_avatars_caps'] ) ? 0 : 1;
		return $new_input;
	}
	/**
	...
	public function edit_user_profile( $profileuser ) {
		...
		<h3><?php _e( 'Profile Photo', 'basic-user-avatars' ); ?></h3>
		<table class="form-table">
			<tr>
				...
				<?php
				$options = get_option( 'dispute_user_avatars_caps' );
				if ( empty( $options['dispute_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					// Nonce security ftw
		...
		<form id="basic-user-avatar-form" action="<?php the_permalink(); ?>" method="post" enctype="multipart/form-data">
			...
			if ( empty( $options['dispute_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
				...
				if ( empty( $profileuser->basic_user_avatar ) ) {
					...
					echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'basic-user-avatars' ) . '<br />';
					echo '<p class="description">' . __( 'Replace the profile photo by uploading a new one, or erase the current photo (falling back to a gravatar) by checking the delete option.', 'basic-user-avatars' ) . '</p>';
				}
			} else {
				if ( empty( $profileuser->basic_user_avatar ) ) {
	...
	public function bbpress_user_profile() {
		...
		$profileuser = get_userdata( $user_id );
		echo '<div>';
			echo '<label for="basic-local-avatar">' . __( 'Profile Photo', 'basic-user-avatars' ) . '</label>';
...
$dispute_user_avatars = new dispute_user_avatars;
/**
 * During uninstallation, remove the custom field from the users and delete the local avatars
 *
 * @since 1.0.0
 */
function dispute_user_avatars_uninstall() {
	$dispute_user_avatars = new dispute_user_avatars;
	$users = get_users_of_blog();
	foreach ( $users as $user )
		$dispute_user_avatars->avatar_delete( $user->user_id );
	delete_option( 'dispute_user_avatars_caps' );
}

register_uninstall_hook( __FILE__, 'dispute_user_avatars_uninstall' );